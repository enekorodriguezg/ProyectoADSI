...
def iu_lpokemon_blueprint(db):
    ...
    # --- NUEVA RUTA: LOGIN (REQUISITO PARA LISTAP-01) ---
    ...
    # --- NUEVA RUTA: LOGOUT (REQUISITO PARA LISTAP-18) ---
    @bp.route('/logout')
    def logout():
        session.pop('user', None)
        flash('Sesión cerrada correctamente.', 'success')
        # Redirige al menú de inicio (simulado como lpokemon.login)
        return redirect(url_for('lpokemon.login'))

    @bp.route('/lpokemon')
    def mostrarLista():
        # --- MEJORA DE SEGURIDAD (LISTAP-01) ---
        # Si el usuario no ha iniciado sesión, redirigimos al login
        if 'user' not in session:
            flash('Acceso no permitido. Por favor, inicie sesión.', 'error')
            return redirect(url_for('lpokemon.login'))
    ...
    def mostrarDetalle(id_pokemon):
        ...
        if 'user' in session:
            ...
            if res_fav.next():
                val = res_fav.getInt('fav_pokemon')
                if val and val > 0:
                    id_fav_actual = val
